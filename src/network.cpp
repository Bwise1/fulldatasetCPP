...
{
    // Step 1: Compare scalar integer members
    if (network1->num_inputs != network2->num_inputs ||
        network1->num_hidden != network2->num_hidden ||
        network1->num_outputs != network2->num_outputs ||
        network1->train_dataset_size != network2->train_dataset_size ||
        network1->test_dataset_size != network2->test_dataset_size)
    {
        return false;
    }

    // Step 2: Compare input-to-hidden weights (wih)
    for (int i = 0; i < network1->num_inputs; ++i)
    {
        for (int j = 0; j < network1->num_hidden; ++j)
        {
            if (network1->wih[i][j] != network2->wih[i][j])
            {
                return false;
            }
        }
    ...
    // Step 3: Compare hidden-to-output weights (who)
    for (int i = 0; i < network1->num_hidden; ++i)
    {
        for (int j = 0; j < network1->num_outputs; ++j)
        {
            if (network1->who[i][j] != network2->who[i][j])
            {
                return false;
            }
        }
    ...
    // Step 4: Compare hidden layer biases (bih)
    for (int i = 0; i < network1->num_hidden; ++i)
    {
        if (network1->bih[i] != network2->bih[i])
        {
            return false;
        }
    }

    // Step 5: Compare output layer biases (bho)
    for (int i = 0; i < network1->num_outputs; ++i)
    {
        if (network1->bho[i] != network2->bho[i])
        {
            return false;
        }
    }

    // Step 6: All members are equal, return true
    return true;
}
